// ============================================
// HomePlate — USER APP  (v3 — bug-fixed & premium)
// Bottom-nav: Home / Orders / Cart / Profile
// Multi-address + Live location (Leaflet + Nominatim, free)
// ============================================
//  Bug fixes vs v2:
//   • Form fields read via querySelector('[name="x"]') → no more shadowing
//   • Map now lives in a dedicated #map-el child (Leaflet no longer clashes
//     with the pin overlay), and invalidateSize() runs post-transition.
//   • _dishCache is populated on ALL views (home, chefs, chef-detail)
//     so add / +/- refresh works everywhere.
//   • refreshDishCard rewritten — never re-JSON-parses data-attr, uses cache.
//   • RTDB onValue subscription is now unsubscribed on route change.
//   • requireAuth() no longer races the outer auth listener.
//   • Cart qty controls update in place instead of full re-render.
//   • Every button gets a ripple + haptic-ish scale animation.
// ============================================
...
const loaderHTML = (label='Warming up the kitchen…') => `
  <div class="full-loader"><div class="cooking-loader">
    ...
    <div class="pan"></div><div class="label">${escapeHtml(label)}</div>
  </div></div>`;
...
const clearCart = () => { localStorage.removeItem(CART_KEY); updateBadges(); window.dispatchEvent(new Event('cart-updated')); };
...
onAuthStateChanged(auth, u => {
  _user = u;
  _authReady = true;
  _authWaiters.splice(0).forEach(fn => fn(u));
  renderHeader();
  renderBottomNav(activeTabFromRoute());
});
const waitForAuth = () => new Promise(res => {
  if(_authReady) return res(_user);
  _authWaiters.push(res);
});
...
// ============================================
// PREMIUM UI: ripple on every clickable
// ============================================
document.addEventListener('click', e => {
  const btn = e.target.closest('.btn, .icon-btn, .nav-item, .dish-add-btn, .qty-inline button, .qty-controls button, .filter-chip, .tag, .auth-tab, .category-card, .checkout-fab, .address-action-btn, .saved-addr-item, .profile-menu-item');
  ...
  h.innerHTML = `
    <div class="header-inner">
      <div class="header-row-1">
        ...
        ${_user
          ? `<div class="header-avatar" id="header-avatar" title="${escapeHtml(_user.email||'')}">${initial}</div>`
      ...
      <div class="search-bar" id="header-search">
        ...
        <i class="fa-solid fa-microphone search-mic" title="Voice search (coming soon)"></i>
      </div>
    </div>`;
...
// BOTTOM NAV
// ============================================
function activeTabFromRoute(){
  const p = parseRoute().seg[0] || 'home';
  const map = { home:'home', chefs:'home', chef:'home', cart:'cart', orders:'orders', profile:'profile' };
  return map[p] || 'home';
}
...
function openAddressSheet(){
  ...
  body.innerHTML = `
    ...
    <button class="address-action-btn primary" id="use-gps-btn">
      <i class="fa-solid fa-location-crosshairs"></i>
      <div class="btn-body">
        <div>Use my current location</div>
        <small>Uses your device GPS</small>
      </div>
      <i class="fa-solid fa-chevron-right"></i>
    </button>
    <button class="address-action-btn ghost" id="add-new-addr-btn">
      <i class="fa-solid fa-plus"></i>
      <div class="btn-body"><div>Add a new address</div></div>
      <i class="fa-solid fa-chevron-right"></i>
    </button>
    <h4 class="saved-title">Saved addresses</h4>
    ${addrs.length ? addrs.map(a => `
      ...
      </div>`).join('') : `<p class="muted-center">No saved addresses. Add your first one!</p>`}
  `;
  ...
  body.querySelectorAll('[data-select]').forEach(el => el.onclick = e => {
    ...
    setSelectedAddr(el.dataset.select);
    sheet.classList.remove('open');
    toast('Delivery address updated');
    renderHeader();
    if(parseRoute().seg[0]==='cart') viewCart();
  });
...
function openMapSheet(editAddr=null){
  ...
  setTimeout(() => {
    ...
    const mapEl = $('#map-el');
    if(!mapEl) return;
    _map = L.map(mapEl, { zoomControl:true, attributionControl:true }).setView([initLat, initLng], 16);
    ...
    setTimeout(()=>_map && _map.invalidateSize(), 600);
  }, 120);
}
...
function tryGetLocation(silent){
  ...
  if(!silent) toast('Fetching location…','info',1500);
...
async function reverseGeocode(lat, lng){
  const tEl = $('#map-addr-title');
  const sEl = $('#map-addr-sub');
  if(tEl) tEl.textContent = 'Locating…';
  if(sEl) sEl.textContent = `${lat.toFixed(4)}, ${lng.toFixed(4)}`;
  ...
  _geoTimer = setTimeout(async () => {
    try {
      ...
      if(tEl) tEl.textContent = short;
      if(sEl) sEl.textContent = addr;
    } catch(e){
      _mapAddress.fullAddress = `${lat.toFixed(4)}, ${lng.toFixed(4)}`;
      if(tEl) tEl.textContent = 'Custom location';
      if(sEl) sEl.textContent = _mapAddress.fullAddress;
    }
  }, 400);
}
...
$('#save-address-btn')?.addEventListener('click', () => {
  ...
  saveAddresses(list);
  // If nothing is selected, or the selected id is gone, pick the newest
...
// ============================================
// DISH CACHE (populated everywhere)
// ============================================
const _dishCache = {};
const cacheDish  = d => { _dishCache[d.id] = d; };
...
function chefCardHTML(chef){
  return `
    <a href="#/chef/${chef.id}" class="chef-card">
      <div class="chef-cover-wrap">
        <img class="chef-cover" src="${escapeHtml(chef.cover||chef.photo||FALLBACK_CHEF)}" alt="${escapeHtml(chef.name)}" loading="lazy" onerror="this.src='${FALLBACK_CHEF}'" />
      ...
      <div class="chef-card-body">
        ...
        <div class="chef-meta">
          <span class="rating-pill"><i class="fa-solid fa-star"></i> ${escapeHtml(String(chef.rating||'4.8'))}</span>
          <span class="dot">•</span>
          <span><i class="fa-solid fa-bolt"></i> ${escapeHtml(String(chef.deliveryTime||45))} min</span>
          <span class="dot">•</span>
...
function dishCardHTML(dish){
  ...
  return `
    <div class="dish-card" data-dish-card="${dish.id}">
      <div class="dish-info">
        ...
        <div class="dish-price-row">
          ...
          <span class="dish-rating"><i class="fa-solid fa-star"></i> ${escapeHtml(String(dish.rating||'4.7'))}</span>
        </div>
        <div class="dish-desc">${escapeHtml(dish.description||'A warm, hand-crafted home-style dish.')}</div>
      </div>
      <div class="dish-thumb-wrap">
        <img class="dish-thumb" src="${escapeHtml(dish.image||FALLBACK_DISH)}" alt="${escapeHtml(dish.name)}" loading="lazy" onerror="this.src='${FALLBACK_DISH}'" />
        <div class="dish-action" data-dish-action="${dish.id}">
          ${inCart
            ? `<div class="qty-inline">
                 <button data-qty-dec="${dish.id}" aria-label="Remove one"><i class="fa-solid fa-minus"></i></button>
                 <span data-qty-count="${dish.id}">${inCart.qty}</span>
                 <button data-qty-inc="${dish.id}" aria-label="Add one"><i class="fa-solid fa-plus"></i></button>
               </div>`
            : `<button class="dish-add-btn" data-add="${dish.id}">ADD <i class="fa-solid fa-plus"></i></button>`}
        </div>
      </div>
    </div>`;
}

function wireDishActions(scope=document){
  scope.querySelectorAll('[data-add]').forEach(b => b.onclick = e => {
    e.preventDefault();
    const id = b.dataset.add;
    const d = _dishCache[id];
    if(!d) return;
    addToCart(d);
    b.classList.add('added');
    b.innerHTML = '✓ ADDED';
    setTimeout(() => refreshDishActions(id), 320);
  });
  scope.querySelectorAll('[data-qty-inc]').forEach(b => b.onclick = e => {
    e.preventDefault();
    const id = b.dataset.qtyInc;
    const d = _dishCache[id];
    if(!d) return;
    addToCart(d);
    refreshDishActions(id);
  });
  scope.querySelectorAll('[data-qty-dec]').forEach(b => b.onclick = e => {
    e.preventDefault();
    updateQty(b.dataset.qtyDec, -1);
    refreshDishActions(b.dataset.qtyDec);
  });
}

/**  In-place refresh of every visible action block for a dish
 *   — no full re-render, avoids scroll jumps and flicker. */
function refreshDishActions(dishId){
  const d = _dishCache[dishId];
  document.querySelectorAll(`[data-dish-action="${dishId}"]`).forEach(box => {
    const inCart = getCart().find(i=>i.id===dishId);
    box.innerHTML = inCart
      ? `<div class="qty-inline">
           <button data-qty-dec="${dishId}" aria-label="Remove one"><i class="fa-solid fa-minus"></i></button>
           <span data-qty-count="${dishId}">${inCart.qty}</span>
           <button data-qty-inc="${dishId}" aria-label="Add one"><i class="fa-solid fa-plus"></i></button>
         </div>`
      : `<button class="dish-add-btn" data-add="${dishId}">ADD <i class="fa-solid fa-plus"></i></button>`;
    wireDishActions(box);
  });
}
...
async function viewHome(){
  ...
  root.innerHTML = `
    <section class="hero-banner">
      <div class="hero-banner-content">
        <div class="hero-eyebrow"><i class="fa-solid fa-heart"></i> Home-cooked with love</div>
        <h1>Real home-cooked meals<br/><span>in 45 minutes</span></h1>
  ...
  if(!chefsSnap || chefsSnap.empty){
    chefsBox.innerHTML = `<p class="empty-inline">No chefs yet. Check back soon.</p>`;
  } else {
    const chefs = []; chefsSnap.forEach(d => chefs.push({ id:d.id, ...d.data() }));
    chefs.sort((a,b)=>(b.rating||0)-(a.rating||0));
    chefsBox.innerHTML = chefs.slice(0,6).map(chefCardHTML).join('');
  }
  ...
  if(!dishesSnap || dishesSnap.empty){
    ...
    const dishes = []; dishesSnap.forEach(d => { const dat = { id:d.id, ...d.data() }; dishes.push(dat); cacheDish(dat); });
...
async function viewChefs(qs){
  ...
  try {
    ...
    $('#chef-list').innerHTML = `<p class="error-cell">Failed to load: ${escapeHtml(e.message)}</p>`;
    return;
  }
  ...
  const render = () => {
    ...
    grid.innerHTML = list.map(chef => `
      <a href="#/chef/${chef.id}" class="chef-list-card">
        <img class="chef-list-cover" src="${escapeHtml(chef.cover||chef.photo||FALLBACK_CHEF)}" alt="${escapeHtml(chef.name)}" loading="lazy" onerror="this.src='${FALLBACK_CHEF}'" />
        <span class="chef-offer-badge">20% OFF</span>
        <div class="chef-list-body">
          ...
          ${chef.signature ? `<div class="chef-signature"><i class="fa-solid fa-utensils"></i>${escapeHtml(chef.signature)}</div>` : ''}
          <div class="chef-list-meta">
            <span class="rating-pill"><i class="fa-solid fa-star"></i> ${escapeHtml(String(chef.rating||'4.8'))}</span>
            <span class="time-pill"><i class="fa-solid fa-bolt"></i> 45 min</span>
            <span class="muted"><i class="fa-solid fa-bag-shopping"></i> ${escapeHtml(String(chef.orders||'500+'))}</span>
          </div>
        </div>
      </a>`).join('');
  };
...
async function viewChefDetail(chefId){
  ...
  try {
    ...
    const dishes = []; dishSnap.forEach(d => { const dat = { id:d.id, ...d.data(), chefName:chef.name, chefId:chef.id }; dishes.push(dat); cacheDish(dat); });
    ...
    root.innerHTML = `
      <button class="icon-btn chef-back-btn no-ripple" onclick="history.back()"><i class="fa-solid fa-arrow-left"></i></button>
      <div class="chef-hero" style="background-image:url('${escapeHtml(cover)}')">
      ...
      ` : categories.map(cat => `
        <div>
          ...
          <div class="dish-grid menu-items">
            ${dishes.filter(d => (d.category||'Mains')===cat).map(d => dishCardHTML(d)).join('')}
          </div>
        </div>
      `).join('')}
      ...
      <div class="reviews-block">
        ...
        ${reviews.length ? reviews.map(r => `
          ...
          </div>`).join('') : `<p class="muted-center">No reviews yet — be the first!</p>`}
      </div>
    `;

    wireDishActions();
  } catch(e){
    root.innerHTML = `<p class="error-cell">${escapeHtml(e.message)}</p>`;
  }
}
...
const emptyStateHTML = (title, desc, href, cta) => `
  <div class="empty-state">
    ...
    ${href?`<a href="#${href}" class="btn btn-primary btn-lg">${escapeHtml(cta)}</a>`:''}
  </div>`;
...
function viewCart(){
  ...
  if(!cart.length){
    root.innerHTML = `<div class="page"><div class="page-title">Your cart</div></div>
      <div class="empty-state">
        ...
        <a href="#/" class="btn btn-primary btn-lg"><i class="fa-solid fa-utensils"></i> Browse chefs</a>
      </div>`;
    return;
  }
  ...
  root.innerHTML = `
    ...
    <div class="cart-body">
      ...
      <div class="cart-card">
        <h3><i class="fa-solid fa-bag-shopping"></i> Items</h3>
        <div id="cart-items">${cartItemsHTML(cart)}</div>
      </div>
      ...
      <div class="cart-card">
        ...
        <div class="form-group">
          ...
          <div class="input-wrap">
            ...
            <input class="input has-icon" name="phone" id="phone-input" type="tel" placeholder="+91 98765 43210" value="${escapeHtml(localStorage.getItem('hp_phone')||'')}" />
          </div>
        </div>
      </div>
      ...
      <div class="cart-card">
        <h3><i class="fa-solid fa-receipt"></i> Bill details</h3>
        <div class="summary-row"><span>Item total</span><span id="sum-item">${fmt(subtotal)}</span></div>
        <div class="summary-row"><span>Delivery fee</span><span>${fmt(DELIVERY_FEE)}</span></div>
        <div class="summary-row"><span>Taxes &amp; charges (5%)</span><span id="sum-tax">${fmt(tax)}</span></div>
        <div class="summary-row total"><span>To pay</span><span id="sum-total">${fmt(total)}</span></div>
      </div>
    ...
    <div class="checkout-fab" id="place-order-btn">
      <div class="checkout-fab-total">
        <span id="fab-count">${cart.length} item${cart.length>1?'s':''}</span>
        <strong id="fab-total">${fmt(total)}</strong>
      </div>
      <span class="checkout-cta">Place Order <i class="fa-solid fa-arrow-right"></i></span>
    </div>
  `;
...
function cartItemsHTML(cart){
  return cart.map(item => `
    <div class="cart-item" data-cart-row="${item.id}">
      <img class="cart-item-img" src="${escapeHtml(item.image||FALLBACK_DISH)}" alt="${escapeHtml(item.name)}" onerror="this.src='${FALLBACK_DISH}'" />
      ...
      <div class="cart-item-right">
        <div class="qty-controls">
          <button data-cart-dec="${item.id}" aria-label="Remove one"><i class="fa-solid fa-minus"></i></button>
          <span data-cart-qty="${item.id}">${item.qty}</span>
          <button data-cart-inc="${item.id}" aria-label="Add one"><i class="fa-solid fa-plus"></i></button>
        </div>
        <button class="remove-btn" data-cart-remove="${item.id}"><i class="fa-solid fa-trash-can"></i> Remove</button>
      </div>
    </div>`).join('');
}
...
function bindCartItemActions(){
  const recompute = () => {
    ...
    const tax = Math.round(subtotal * TAX_RATE);
    const total = subtotal + DELIVERY_FEE + tax;
    $('#sum-item')  && ($('#sum-item').textContent  = fmt(subtotal));
    $('#sum-tax')   && ($('#sum-tax').textContent   = fmt(tax));
    $('#sum-total') && ($('#sum-total').textContent = fmt(total));
    $('#fab-total') && ($('#fab-total').textContent = fmt(total));
    $('#fab-count') && ($('#fab-count').textContent = cart.length + ' item' + (cart.length>1?'s':''));
    // update qty cells
    cart.forEach(i => {
      const q = document.querySelector(`[data-cart-qty="${i.id}"]`);
      if(q) q.textContent = i.qty;
    });
  };

  const root = $('#cart-items');
  root.querySelectorAll('[data-cart-inc]').forEach(b => b.onclick = () => {
    updateQty(b.dataset.cartInc, +1); recompute();
  });
  root.querySelectorAll('[data-cart-dec]').forEach(b => b.onclick = () => {
    const id = b.dataset.cartDec;
    const item = getCart().find(i => i.id === id);
    if(item && item.qty === 1){
      const row = document.querySelector(`[data-cart-row="${id}"]`);
      row?.classList.add('removing');
      setTimeout(() => { removeItem(id); toast('Removed from cart'); viewCart(); }, 220);
    } else {
      updateQty(id, -1); recompute();
    }
  });
  root.querySelectorAll('[data-cart-remove]').forEach(b => b.onclick = () => {
...
async function placeOrder(){
  ...
  const cta = btn.querySelector('.checkout-cta');
  const oldCta = cta.innerHTML;
  cta.innerHTML = '<span class="btn-spinner"></span> Placing order…';
...
async function viewTrack(orderId){
  ...
  try {
    ...
    // Unsub previous
    if(_trackRef){ off(_trackRef); _trackRef = null; }
    _trackRef = ref(rtdb,'orderStatus/'+orderId);
    onValue(_trackRef, s => {
      const v = s.val();
      if(v?.status){
        const stepper = document.querySelector('.stepper');
        if(stepper) updateStepperDOM(v.status);
      }
    });
...
function updateStepperDOM(currentStatus){
  ...
  const cd = $('#countdown');
  if(cd && currentStatus === 'Delivered'){ cd.textContent = 'Delivered'; if(_countdownTimer) clearInterval(_countdownTimer); }
}
...
function renderTrack(order, currentStatus){
  ...
  $('#app-root').innerHTML = `
    <div class="track-page">
      <button class="icon-btn track-back-btn no-ripple" onclick="history.back()"><i class="fa-solid fa-arrow-left"></i></button>
      <div class="track-card">
        ...
        <div class="track-details">
          ...
          ${(order.items||[]).map(i => `<div class="det-row"><span>${i.qty} × ${escapeHtml(i.name)}</span><span>${fmt(i.price*i.qty)}</span></div>`).join('')}
          <div class="summary-row total"><span>Total (Cash on Delivery)</span><span>${fmt(order.total)}</span></div>
          <p class="det-foot">
...
function startCountdown(etaIso){
  ...
  const tick = () => {
    ...
    if(el.textContent === 'Delivered') return;
...
async function viewOrders(){
  ...
  try {
    ...
    if(!orders.length){
      container.innerHTML = `<div class="empty-state"><div class="empty-illust"><i class="fa-solid fa-receipt"></i></div><h3>No orders yet</h3><p>Your order history will appear here.</p><a href="#/" class="btn btn-primary btn-lg"><i class="fa-solid fa-utensils"></i> Browse chefs</a></div>`;
      return;
    }
      ...
      ${orders.map(o => {
        ...
        return `
          <div class="order-card">
            ...
            <div class="order-items-preview"><i class="fa-solid fa-utensils"></i> ${items}</div>
            ...
            <div class="order-foot">
              ...
              <div class="order-actions">
                ...
                <button class="btn btn-ghost btn-sm" data-reorder="${o.id}"><i class="fa-solid fa-rotate-right"></i> Reorder</button>
    ...
    $('#orders-root').innerHTML = `<p class="error-cell">${escapeHtml(e.message)}</p>`;
  }
}
...
function wireReviewModal(){
  ...
  $('#submit-review')?.addEventListener('click', async () => {
    ...
    try {
      ...
      try { await updateDoc(doc(db,'orders',selectedOrderId), { reviewed:true }); } catch(_){}
...
function viewLogin(qs){
      ...
      <form id="auth-form" autocomplete="on">
        <div class="form-group" id="name-group" style="display:none;">
          ...
          <div class="input-wrap">
            ...
            <input class="input has-icon" name="name" type="text" placeholder="Your name" />
          </div>
        </div>
        <div class="form-group">
          <label>Email</label>
          <div class="input-wrap">
            ...
            <input class="input has-icon" name="email" type="email" placeholder="you@example.com" required autocomplete="email" />
          </div>
        </div>
        <div class="form-group">
  ...
  $$('.auth-tab').forEach(t => t.onclick = () => {
    ...
    $('#submit-btn').innerHTML = mode==='signup'
...
function route(){
  ...
  if(_trackRef && page !== 'track'){ off(_trackRef); _trackRef = null; }
  window.scrollTo(0,0);
